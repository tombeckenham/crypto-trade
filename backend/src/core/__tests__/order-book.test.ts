...
describe('OrderBook', () => {
  ...
  describe('Initialization', () => {
    it('should create empty order book with correct pair', () => {
      ...
      expect(orderBook.getSpread()).toBe(Infinity.toString());
    });
  });
  ...
  describe('Order Management', () => {
    ...
    it('should aggregate orders at same price level', () => {
      ...
      const order3 = createOrder('buy', "50000", "2");
  ...
  describe('Best Bid/Ask Logic', () => {
    ...
    it('should return highest bid as best bid', () => {
      ...
      expect(parseFloat(bestBid!.price)).toBeGreaterThanOrEqual(49500);
      expect(parseFloat(bestBid!.amount)).toBeGreaterThan(0);
    });
    ...
    it('should calculate spread correctly', () => {
      ...
      expect(parseFloat(spread)).toBeGreaterThan(0); // Should be positive spread
      expect(parseFloat(spread)).toBeLessThan(2000); // Should be reasonable
    });
  });
  ...
  describe('Market Depth', () => {
    ...
    it('should generate correct market depth', () => {
      ...
      expect(depth.bids[0]!.amount).toBe("1");
      expect(depth.bids[0]!.total).toBe("1");
      ...
      expect(depth.bids[2]!.amount).toBe("2");
      ...
      expect(depth.asks[1]!.total).toBe("2"); // 0.8 + 1.2
  ...
  describe('Volume Calculations', () => {
    ...
    it('should calculate total sell volume', () => {
      ...
      expect(sellVolume).toBe("2"); // 0.8 + 1.2
    });
  ...
  describe('Performance Tests', () => {
    it('should handle large number of orders efficiently', () => {
      ...
      for (let i = 0; i < numOrders; i++) {
        ...
        orderBook.addOrder(createOrder(side, price.toString(), amount.toString()));
      }
    ...
    it('should maintain performance during mixed operations', () => {
      ...
      for (let i = 0; i < operations; i++) {
        ...
        if (operation < 0.6 || orderIds.length === 0) {
          ...
          const order = createOrder(side, price.toString(), amount.toString());
          ...
          orderBook.updateOrderAmount(orderId, fillAmount.toString());
        }
      }
  ...
  describe('Edge Cases and Error Handling', () => {
    ...
    it('should handle very large numbers', () => {
      ...
      const order = createOrder('buy', largePrice.toString(), largeAmount.toString());

      orderBook.addOrder(order);

      const bestBid = orderBook.getBestBid();
      expect(bestBid!.price).toBe(largePrice.toString());
      expect(bestBid!.amount).toBe(largeAmount.toString());
    });

    it('should handle very small decimal amounts', () => {
      const smallAmount = 0.00000001;
      const order = createOrder('buy', "50000", smallAmount.toString());

      orderBook.addOrder(order);

      const bestBid = orderBook.getBestBid();
      expect(bestBid!.amount).toBe(smallAmount.toFixed(8));
    });
  ...
  describe('Real Trading Scenarios', () => {
    it('should handle typical market making scenario', () => {
      ...
      for (let i = 1; i <= 5; i++) {
        ...
        orderBook.addOrder(createOrder('buy', bidPrice.toString(), amount.toString()));
        orderBook.addOrder(createOrder('sell', askPrice.toString(), amount.toString()));
      }

      expect(orderBook.getOrderCount()).toBe(10);
      expect(orderBook.getSpread()).toBe((spread + 40).toString()); // spread + 2 * first level offset
    ...
    it('should handle partial fills correctly', () => {
      ...
      orderBook.updateOrderAmount(order.id, "1.0");
      expect(orderBook.getBestBid()!.amount).toBe("4");
      expect(orderBook.getOrder(order.id)!.status).toBe('partial');

      orderBook.updateOrderAmount(order.id, "3.0");
      expect(orderBook.getBestBid()!.amount).toBe("2");
      expect(orderBook.getOrder(order.id)!.status).toBe('partial');

      // Fully fill the order
      orderBook.updateOrderAmount(order.id, "5");
      expect(orderBook.getBestBid()).toBe(null);
      expect(orderBook.getOrder(order.id)).toBe(undefined);
    });
  });
